...
export default function MDAsList() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedMDA, setSelectedMDA] = useState<MDA | null>(null);
  const [deleteConfirm, setDeleteConfirm] = useState<MDA | null>(null);
  const queryClient = useQueryClient();

  const { data, isLoading } = useQuery({
    queryKey: ['mdas', page, search],
    queryFn: async () => {
      const response = await mdasAPI.getAll({ page, limit: 10, search });
      return response.data;
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => mdasAPI.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['mdas'] });
      toast.success('MDA deleted successfully');
      setDeleteConfirm(null);
    },
    onError: () => {
      toast.error('Failed to delete MDA');
    },
  });
  ...
  const columns: Column<MDA>[] = [
    {
      ...
      render: (item) => (
        <div>
          ...
          {item.acronym && <p className="text-sm text-gray-500">{item.acronym}</p>}
        </div>
      ),
    },
    {
    ...
    },
    {
      key: 'head',
      label: 'Head',
      render: (item) => item.head?.name || '-',
    },
    {
      key: 'contact',
      label: 'Contact',
      render: (item) => item.contactInfo?.email || item.contactInfo?.phone || '-',
    },
    {
      key: 'isActive',
      label: 'Status',
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">MDAs Management</h1>
          <p className="text-gray-600 mt-1">Manage Ministries, Departments, and Agencies</p>
        </div>
        <button
      ...
      <Modal
        ...
        title={selectedMDA ? 'Edit MDA' : 'Add MDA'}
        size="xl"
      >
        <MDAForm
          mda={selectedMDA}
