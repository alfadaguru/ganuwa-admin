...
test.describe('Extract Data from React Source Files', () => {
  const sourceBasePath = '/Users/aliyumohammedlawal/Documents/Projects - New/kanostate.gov.ng/source code/kano-state-website/src';
  const outputDir = '/tmp';
  ...
  test('Extract News Articles from NewsPage.tsx', async () => {
    const filePath = path.join(sourceBasePath, 'pages/NewsPage.tsx');
    const fileContent = fs.readFileSync(filePath, 'utf-8');

    // Extract news articles array (lines 34-143)
    const newsArticles = [
      {
    ...
    const transformedNews = newsArticles.map(article => {
      // Generate full content from excerpt (in real scenario, would be full article)
...
For more information, please contact ${article.author}.`;
  ...
  test('Extract Press Releases from NewsPage.tsx', async () => {
    const pressReleases = [
      {
    ...
    // Transform to backend schema (as news with pressRelease category)
...
For media inquiries, please contact the Ministry of Information.`;
  ...
  test('Extract MDAs from MDAsPage.tsx', async () => {
    const ministries = [
      {
    ...
    const transformedAgencies = agencies.map(agency => {
      // Extract acronym from name (if in parentheses) or generate
  ...
  test('Extract LGAs from LGAsPage.tsx', async () => {
    const lgas = [
  ...
  test('Extract Services from ServicesPage.tsx', async () => {
    const services = [
      {
    ...
    const transformedServices = services.map(service => {
      ...
      return {
        ...
        status: service.status === 'online' ? 'active' : 'active',
        isActive: true,
      };
    });
  ...
  test('Extract Leaders from ExecutivePage.tsx and LeadershipSection.tsx', async () => {
    const leaders = [
      {
  ...
  test('Extract Projects from ProjectsPage.tsx', async () => {
    const projects = [
      {
